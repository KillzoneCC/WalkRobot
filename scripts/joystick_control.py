...
import time
import rospy
import numpy as np # Добавлено для работы с IMU данными
from ainex_sdk import Board
from sensor_msgs.msg import Joy, Imu # Добавлено Imu
from ainex_kinematics.gait_manager import GaitManager
from ainex_kinematics.motion_manager import MotionManager # Добавлено MotionManager
...
class JoystickController:
    ...
    def imu_callback(self, msg):
        ...
        # Значения могут немного отличаться в зависимости от калибровки IMU
        # Ориентировочные значения для акселерометра при лежании:
        # Лицом вниз (prone): ax ~ 9.8, ay ~ 0, az ~ 0
        # Лицом вверх (supine): ax ~ -9.8, ay ~ 0, az ~ 0
        # Стоя (standing): ax ~ 0, ay ~ 0, az ~ 9.8
        
        # Допуск для сравнения с гравитацией
        tolerance = 2.0 

        if abs(ax - 9.8) < tolerance and abs(ay) < tolerance:
            # Робот лежит лицом вниз
            if self.robot_state != 'lie':
                rospy.loginfo("Robot state: Lying Face Down (prone)")
            self.robot_state = 'lie'
        elif abs(ax + 9.8) < tolerance and abs(ay) < tolerance:
            # Робот лежит лицом вверх
            if self.robot_state != 'recline':
                rospy.loginfo("Robot state: Lying Face Up (supine)")
            self.robot_state = 'recline'
        else:
            # Робот стоит
            if self.robot_state != 'stand':
                rospy.loginfo("Robot state: Standing")
    ...
    def circle_callback(self, new_state):
        # Активация функции "Get up" по кнопке Circle
        if new_state == ButtonState.Pressed:
            rospy.loginfo(f"Circle button pressed. Robot state: {self.robot_state}")
